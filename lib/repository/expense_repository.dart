...
class ExpenseRepository extends Adapter<Expense> {
  ...
  Future<double> getSumByCategory(CategoryEnum value) async {
    return await isar.expenses
        .filter()
  ...
  Future<List<Expense>> getObjectsByAmountRange(double min, double max) async {
    return await isar.expenses.filter().amountBetween(min, max).findAll();
  }

  Future<List<Expense>> getObjectsWithAmountGreaterThan(double value) async {
    return await isar.expenses.filter().amountGreaterThan(value).findAll();
  }

  Future<List<Expense>> getObjectsWithAmountLessThan(double value) async {
    return await isar.expenses.filter().amountLessThan(value).findAll();
  }
